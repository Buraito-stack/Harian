...
export function LoginPage({ onSwitchToRegister }: LoginPageProps) {
  ...
  const validate = (): boolean => {
    ...
    // No email format validation - allow nickname too
    ...
    if (!email.trim() && !password) {
      setErrors({ ...newErrors, general: "Email/Nickname dan password wajib diisi" });
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Clear previous errors only, keep the input values
    setErrors({});
    ...
    try {
      ...
      let message = t.errors.unauthorized;
      ...
      // Only clear password on failed login, keep email
      setPassword("");
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background px-4 relative">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className={`absolute -top-40 -right-40 w-80 h-80 rounded-full blur-3xl ${isDark ? 'bg-mornye/20' : 'bg-mornye-light/20'}`} />
        <div className={`absolute -bottom-40 -left-40 w-80 h-80 rounded-full blur-3xl ${isDark ? 'bg-starlight/10' : 'bg-mornye-light/10'}`} />
      </div>
        ...
        <Card variant="elevated" padding="lg">
          <CardContent className="mt-0">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <Input
                label="Email atau Nickname"
                type="text"
                placeholder="email@example.com atau nickname"
                value={email}
